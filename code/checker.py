...
def fwd(network, valueDict):
    ad = Autograd(network.graph)
    return ad.eval(network.graph.operationSequence(network.graph.loss), valueDict)

def bwd(network, valueDict):
    ad = Autograd(network.graph)
    return ad.bprop(network.graph.operationSequence(network.graph.loss), valueDict,loss=np.float_(1.0))
...
if __name__ == '__main__':
    ...
    print _crossEnt(np.load(output_file+".npy"), np.load(output_file+".npy")).mean()
    # print "building lstm..."
