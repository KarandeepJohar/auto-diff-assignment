...
def _crossEnt(x,y):
    ...
    return - np.multiply(y,log_x).sum()
...
BP_FUNS = {
    ...
    'mean':             [lambda delta,out,x : delta * 1.0/float(x.shape[0])],
...
def MLP2():
    ...
    x.W1 = f.param()
    x.b1 = f.param()
    x.o1 = f.tanh( x.input*x.W1 + x.b1 )
    # x.W2 = f.param()
    # x.b2 = f.param()
    # x.o2 = f.tanh( x.o1*x.W2 + x.b2 )
    # x.W3 = f.param()
    # x.b3 = f.param()
    # x.o3 = f.tanh( x.o2*x.W3 + x.b3 )
...
if __name__ == "__main__":
    ...
    fd = learn(LinearRegression, epochs, rate=1.0, input=x, weights=nullWeights, y=px)
    ...
    fwd = learn(MLP, epochs=100, input=x, rate=0.001, 
            W1=W1,
            b1=b1,
            W2=W2,
            b2=b2,
    ...
    h = MLP2()
    ad = Autograd(h)
    # X_test = parse_images("train-images.idx3-ubyte")
    # y_test = parse_labels("train-labels.idx1-ubyte")
    ...
    #fwd = learn(MLP, epochs=100, input=X_test, rate=0.0005, 
    #        W1=W1,
    #        b1=b1,
    #        W2=W2,
    #        b2=b2,
    #        y=y_test)
    # print "kkkkkkkkkkkkkkkkkk"
    # print X_test.shape, fwd['W1'].shape
    # fpd = ad.eval(h.operationSequence(h.output), 
    #         h.inputDict(input=X_test, W1=fwd['W1'], b1=fwd['b1'], W2=fwd['W2'], b2=fwd['b2']))
    # print 'learned/target predictions, MLP'
